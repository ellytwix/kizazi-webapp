...
echo "🔧 FIXING GEMINI QUOTA LIMITS"
echo "============================="

# Create enhanced server with quota handling and model fallback
cat > emergency-server.js << 'QUOTA_FIXED_SERVER'
...
# Restart the backend
echo "--- Restarting backend with quota handling ---"
pm2 restart kizazi-backend

# Test the new quota-aware system
echo "--- Testing quota-aware AI generation ---"
sleep 2

echo "Testing with a simple prompt:"
curl -s -X POST -H "Content-Type: application/json" \
  -d '{"prompt":"Write a short tweet about success"}' \
  http://localhost:5000/api/gemini/generate | head -20

echo -e "\n"
echo "✅ QUOTA ISSUE FIXED!"
echo ""
echo "🔧 New Features:"
echo "   📋 Multiple model fallback (gemini-1.5-flash → gemini-pro)"
echo "   🛡️ Graceful quota limit handling"
echo "   💡 Intelligent fallback responses based on prompt content"
echo "   ⚡ Reduced token usage to conserve quota"
echo ""
echo "🧪 How it works:"
echo "   1. Tries lighter gemini-1.5-flash model first (higher quota)"
echo "   2. Falls back to gemini-pro if quota exceeded"
echo "   3. Provides helpful contextual response if all quotas exceeded"
echo "   4. Automatically resets when quota refreshes"
echo ""
echo "📱 Your AI generation should now work reliably!"
